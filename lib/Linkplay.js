...
class Linkplay {
        ...
        let status = await this.command('getPlayerStatus', true)
            ...
            .then(async status => {
                if (status.resolvedMode === 'PLAYER_MODE_SPOTIFY') {
                    ...
                    if (upnpStatus) {
                        status.Title = _.get(upnpStatus, 'dc:title', 'Unknown').toString();
                        status.Artist = _.get(upnpStatus, 'upnp:artist', 'Unknown').toString();
                        status.Album = _.get(upnpStatus, 'upnp:album', 'Unknown').toString();
                        let albumArtUri = _.get(upnpStatus, 'upnp:albumArtURI', false);
                        if (typeof albumArtUri === 'string')
                            albumArtUri = albumArtUri.replace('http://', 'https://')
                        status.albumArtUri = albumArtUri;
                    }
                }
                return status;
            })
            .catch(e => {
                this.log('Error getting player status: ', e)
            });
        if (status)
    ...
    playUrl(url) {
        return this.setPlayerCommand('play', url);
    }
